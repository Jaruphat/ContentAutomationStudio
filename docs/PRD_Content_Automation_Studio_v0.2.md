...
| เวอร์ชันเอกสาร | 0.2 — Consolidated Concept and MVP Draft |
...
1. **Creative Brief** — กำหนดเป้าหมาย กลุ่มผู้ชม แพลตฟอร์ม ความยาว hook, key message, CTA และ brand constraints
2. **Create Project** — ตั้งชื่อ project, format, aspect ratio, duration และสไตล์
3. **Story Intake** — ใส่ plot เองหรือสร้างจาก brief
4. **Story Planning** — สร้าง synopsis, acts/beats, characters, locations และ style bible
5. **Scene Breakdown** — แตกเป็น scenes โดยกำหนดวัตถุประสงค์ เหตุการณ์ ตัวละคร สถานที่ และ duration
6. **Shot Planning** — แตก scene เป็น shots พร้อม shot type, camera, action, dialogue และ duration
7. **Storyboard Review** — ผู้ใช้แก้ไข ล็อก และอนุมัติ shot list
8. **Prompt Compilation** — รวม shot prompt กับ character/location/style constraints
9. **Cost & Capacity Preview** — สรุปจำนวน jobs/takes, VRAM profile, เวลาประมาณการ และพื้นที่จัดเก็บ
10. **Generation Planning** — เลือก image/video workflow, preset, seed policy และ reference assets
11. **Queue Execution** — ส่ง jobs ไป ComfyUI และติดตามสถานะ
12. **Take Review** — แสดง outputs หลาย take ให้ approve/reject/regenerate
13. **Timeline Assembly** — นำ approved takes เรียงตาม shot order
14. **Audio/Subtitles** — เพิ่ม voice, ambience, music, captions ตามที่มี
15. **Proxy Preview** — render แบบ low-cost สำหรับอนุมัติจังหวะและ take
16. **Final Conform** — ใช้ approved source assets ตาม Timeline Manifest
17. **QC Review** — ตรวจ missing shot, duration, aspect ratio, black frame, audio และ subtitle
18. **Content Package Export** — video, shorts, storyboard, captions, manifests และ project archive
...
- model/checkpoint/LoRA hash และ input asset hash
...
- subtitle cue
- provenance และ render status

Timeline Manifest เป็น source of truth สำหรับการประกอบวิดีโอ การแก้จังหวะตัดต้องไม่บังคับให้ regenerate ภาพหรือวิดีโอ เว้นแต่ผู้ใช้เปลี่ยน approved take หรือ source asset
...
9. Encode review proxy แล้วรอ approval gate
10. Final conform และ encode master
11. Run automated QC
...
- Interchange: OpenTimelineIO และ EDL สำหรับส่งต่อ DaVinci Resolve/Premiere เมื่อข้อมูลรองรับ
- Content Package: full video, vertical shorts, thumbnail concepts, captions, subtitles, blog outline และ shot reuse map

---

## 14. User Interface

### 14.1 Production Cockpit and Main Navigation
...
หน้าหลักใช้ cockpit 3 ส่วน: ซ้ายเป็น stage rail, กลางเป็น storyboard/timeline grid และขวาเป็น inspector สำหรับ prompt, camera, references, seed, workflow และ take โดยผู้ใช้ไม่จำเป็นต้องเปิด ComfyUI node graph ในงานประจำ
...
|---|---|---|
| FR-01 | สร้าง Creative Brief และบันทึก project จาก wizard ได้ | Must |
...
| FR-21 | แสดง cost/capacity preview และรัน low-res 1-shot test ก่อน batch ได้ | Must |
| FR-22 | ใช้ Timeline Manifest เป็น source of truth และทำ proxy → approval → final conform ได้ | Must |
| FR-23 | Export OpenTimelineIO/EDL ได้เมื่อ timeline fields รองรับ | Should |
| FR-24 | สร้าง Content Package และ shot reuse map จาก Story Bible เดียวกันได้ | Should |
...
- generation cost/capacity preview: jobs, takes, VRAM profile, estimated time และ disk requirement
- ปุ่มทดลอง low-resolution 1 shot ก่อนอนุมัติ batch ใหญ่
...
11. Restart ระหว่าง queue แล้ว resume/reconcile เฉพาะ jobs ที่ค้างได้
12. แก้ timing ใน Timeline Manifest แล้วสร้าง proxy ใหม่ได้โดยไม่ regenerate approved takes
