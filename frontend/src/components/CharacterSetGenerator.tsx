...
export default function CharacterSetGenerator({ projectId }: { projectId: string }) {
  ...
  return <section aria-label="Character Set Generator" className="space-y-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-4"><div><h2 className="text-sm font-semibold text-zinc-100">Character Set Generator</h2><p className="text-xs text-zinc-400">Generate versioned canonical character views, then explicitly approve one version for shot conditioning.</p></div><div className="flex gap-2"><input aria-label="New character set name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Character set name" className="min-w-0 flex-1 rounded px-2 py-1 text-sm" /><button type="button" disabled={!name.trim() || create.isPending} onClick={() => create.mutate()} className="flex items-center gap-1 rounded bg-indigo-600 px-3 py-1 text-xs text-white disabled:opacity-50"><Plus size={12} />Create set</button></div>{query.isLoading && <p className="text-xs text-zinc-400">Loading character sets…</p>}{query.isError && <ActionError label="Load character sets" error={query.error} />}{query.data?.length === 0 && <p className="rounded border border-dashed border-zinc-700 p-4 text-center text-xs text-zinc-500">No character sets yet. Create one to define identity and canonical views.</p>}{query.data?.map((item) => <SetEditor key={item.id} projectId={projectId} value={item} />)}<ActionError label="Create character set" error={create.error} /></section>;
}
